...
router.get('/sessao/:widgetId', async (req: Request, res: Response) => {
  try {
    ...
    // IMPORTANTE: Buscar mensagem inicial do Dialogflow
    let mensagemInicial = null;
    try {
      // Enviar evento de início para o Dialogflow
      const respostaInicial = await dialogflowService.iniciarConversa(
        widget.dialogflow,
        sessionId
      );
      
      if (respostaInicial && respostaInicial.texto) {
        mensagemInicial = respostaInicial.texto;
      }
    } catch (erro) {
      console.log('Usando mensagem padrão, Dialogflow não respondeu:', erro);
      // Se Dialogflow não responder, usar mensagem configurada
      mensagemInicial = widget.mensagens.boasVindas;
    }
    ...
    res.json({ 
      sucesso: true, 
      sessionId,
      mensagemInicial, // Enviar mensagem do Dialogflow
